...
public class StatPrinter {
	...
	// makes frequency of length max(data)
	for (int s = max(data); s >= 0; s--) {
	    _frequency.add(0);
	}
	// adds a number to the index each time it appears in data
	for (Integer i: data) {
    ...
    {
	// uses the reigning champ method
	Integer maxVal = data.get(0);
	for (Integer x:data) {
	    if (x > maxVal) {
		maxVal = x;
	    }
    ...
    {
	// criteria of a local mode
	// could be one line but easier to read this way:
    ...
    {
	// arrayList of all local modes
	ArrayList<Integer> localMode = new ArrayList<Integer>();
	// checks all values in _frequency to see if it is a local mode
	for (int freqs = _frequency.size() - 1; freqs >= 0; freqs--) {
	    if (isLocalMode(freqs)) {
		// add to the arrayList if it is a local mode
		localMode.add(freqs);
	    }
	}
	return localMode;
    ...
    {
	// asterisks per number
	int starsper = longestBar / max(_frequency);
	// goes through the list printing from 0 to _frequency.size() - 1
	for (int i = 0; i < _frequency.size(); i++) {
	    // the header of each row
	    System.out.print(i + " : ");
	    // mults asterisks per number and number at that place for number
	    // of asterisks to print
	    for (int stars = starsper * _frequency.get(i); stars > 0; stars--) {
		System.out.print("*");
	    }
	    System.out.println("");
	}
